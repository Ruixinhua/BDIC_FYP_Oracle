...
def get_reduction_result(model_outputs=None, model_reduction_path=configuration.model_reduction_path,
                         char_types=configuration.char_types):
    if not model_outputs:
        model_outputs = get_model_output()
    # 获取测试结果
    if os.path.exists(model_reduction_path):
        model_reduction_results = pd.read_csv(model_reduction_path)
    ...
    for model_output in model_outputs:
        ...
        index = model_outputs.index(model_output)
        if fea_dim > 512:
            # 使用PCA降至512维
    ...
    model_reduction_results.to_csv(model_reduction_path, encoding="utf-8")
    return model_reduction_results
